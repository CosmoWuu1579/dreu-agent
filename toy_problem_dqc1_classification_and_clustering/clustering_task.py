...
def _system_prompts(dataset_names: list[str], variant: str,
    ...
    if rows:
        ...
        beat_note = (
            ...
            f"start that is the strongest seed, {best_name} (mean ARI "
            f"{best_s['ari']:.3f}, {best_s['gates']} gates, depth "
            f"{best_s['depth']}) -- by matching or beating its mean ARI AND making "
        ...
        baseline_block = (
            ...
            f"success = mean ARI strictly ABOVE it, and a strong result also comes "
            f"in UNDER {best_s['gates']} gates / depth {best_s['depth']} for the "
            f"SAME OR BETTER ARI. First reason about why the strong seeds win and "
            f"weak ones fail, then extend/hybridize/rethink them, always asking "
            f"which gates you can remove.\nSUBMISSION RULE: submit your OWN "
    ...
    return {
        "explore": (
            ...
            f"design of quantum feature maps for UNSUPERVISED entropy-based "
            f"clustering with the DQC1 (one clean qubit) model. The kernel is the "
            f"DQC1 normalized trace K(x,x') = |tr(U(x)U(x')^dag)|/2^n, mapped to "
            f"a similarity M=(K+1)/2, and clusters are grown by minimizing the "
            f"increase in Renyi-2 entropy H2 = -log Tr(rho^2) estimated via DQC1 "
            f"purity. Target datasets: {sets} (2D, non-convex geometry). Use the "
            f"lookup tools (papers, web, seed library) to cover this scope, in "
            f"priority order:\n"
            f"1. General background: current state, known challenges (trace-kernel "
            f"concentration as circuits grow, curved-manifold separation), and "
            f"general insights or recommendations for encoding design.\n"
            f"2. Feature-map comparison: survey the seed library AND encodings "
            f"from prior work that suit a trace kernel (polynomial Pauli-Z phases, "
            f"data re-uploading, radial/angular encodings).\n"
            f"3. Exploratory investigation: this matters MOST -- look beyond "
            f"standard maps (classical kernel ideas, geometry-aware encodings "
            ...
            f"encoding idea, why it should separate THESE datasets under a trace "
            f"kernel, and what to try first.\n"
        ...
        "generate": (
            ...
            f"seed below) and refine it (tweak an encoding, entangler, layer, or "
            f"scale) to beat it: push mean ARI higher and, where you can, use "
            f"fewer gates and lower depth. Prefer improving what already works, "
            f"but it is NOT a cage -- if the best is weak or you have a promising "
            f"idea, a fresh design is fine. (2) SMALL BEATS BIG -- the trace "
            f"kernel CONCENTRATES toward zero as circuits grow, flattening K and "
            f"killing separability; prefer 2-3 qubits, <=2 layers. A bigger "
            f"circuit scoring WORSE means concentration -- shrink, don't grow.\n"
            ...
            f"ARI/NMI + a k-means reference). Submit by replying WITHOUT a tool "
            f"call. Prefer a trial that beats the best, or a promising-but-worse "
            f"trial that opens a useful direction. Resubmitting the current best "
            f"UNCHANGED is a last resort -- justified only when this session's "
            f"trials were genuinely poor or the direction was subpar, never as a "
            f"default. Do NOT resubmit the same design session after session: if "
            f"small tweaks keep failing to beat it, that is your cue to make a "
            f"bold, structurally different change next session, not another "
            f"minor variation."
