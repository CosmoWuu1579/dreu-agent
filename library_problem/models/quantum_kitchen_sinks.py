...
from qml_benchmarks.model_utils import broadcast
...
class QuantumKitchenSinks(BaseEstimator, ClassifierMixin):
    ...
    def construct_model(self):
        ...
        # PennyLane 0.45's unified default.qubit seeds its sampling PRNG via
        # `seed` (which accepts a jax PRNGKey), replacing the old device-specific
        # `prng_key` argument.
        dev = qml.device(
            self.dev_type,
        ...
        def circuit(Q):
            ...
            for i, q in enumerate(Q):
                qml.RX(q, wires=i)
            broadcast(qml.CNOT, wires=range(self.n_qubits_), pattern="double")
            broadcast(qml.CNOT, wires=range(self.n_qubits_), pattern=pattern)
